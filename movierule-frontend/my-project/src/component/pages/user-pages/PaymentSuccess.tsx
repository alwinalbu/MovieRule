    ...
    const patchBooking = async () => {
      ...
      try {
        const response = await commonRequest(
          "PATCH",
          `/bookings/${sessionId}`, // ⚠️ Make sure backend expects sessionId here
          config,
        ...
        setError("Something went wrong while updating your booking.");
      } finally {
        setLoading(false);
      }
    };
  ...
  if (loading) {
    return (
      <div className="h-screen flex justify-center items-center">
        <Spinner size="lg" color="primary" label="Processing Payment..." />
      </div>
    );
  }
