...
const TheatreSeatLayoutEdit = () => {
  ...
  const fetchScreenData = async () => {
    try {
      ...
      // Deep clone so they don't share the same reference
      setLayout(_.cloneDeep(screenData.layout));
      setOriginalLayout(_.cloneDeep(screenData.layout));
  ...
  // 🟢 Toggle Seat (fix restoring logic)
  const handleSeatClick = (rowIndex: number, colIndex: number) => {
    const newLayout = layout.map((row, rowIdx) =>
      row.map((seat, colIdx) => {
        if (rowIdx === rowIndex && colIdx === colIndex) {
          // Toggle between null and original seat
    ...
    <div className="min-h-screen bg-gradient-to-br from-black via-gray-900 to-black text-white p-4 sm:p-6">
      ...
      {loading ? (
        ...
        <>
          ...
          <div className="flex justify-center gap-4 sm:gap-6 mb-6 text-xs sm:text-sm">
            <div className="flex items-center gap-2">
              <img src={seatImg} className="w-4 h-4 sm:w-6 sm:h-6" alt="Available" />
              <span>Available</span>
            </div>
            <div className="flex items-center gap-2">
              <img src={rSeat} className="w-4 h-4 sm:w-6 sm:h-6" alt="Removed" />
              <span>Removed</span>
            </div>
          </div>
