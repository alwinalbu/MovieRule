...
export const theaterGetScreenLayoutController = (
  ...
  return async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      // Fetch screen
      const screen = await getScreenLayoutUseCase(dependencies).execute(
        screenId
      );
      ...
      // 🟢 Return both layouts (with fallback)
      return res.status(200).json({
        layout: screen.layout,
        baseLayout: screen.baseLayout || screen.layout, // ✅ fallback for old screens
      });
    } catch (error: any) {
      console.error("Get screen layout error:", error);
      return res
