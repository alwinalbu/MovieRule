...
export interface IRepositories {
  ...
  updateUserPassword: (data: {email: string, password: string}) => Promise<UserEntity | null>;
   ...
   getMovieByIdRepository:(id: string)=>Promise<IMovie | null>
   toggleWatchlist:(userId: string,movieId: string)=>Promise<{success: boolean;inWatchlist?: boolean;watchlist?: string[]}>;
   getWatchlist: (userId: string) => Promise<IMovie[] | null>;
  };
